...
目前部署包含 2026-06-14 Claude code review 後的修正、2026-06-16 視覺與互動更新、2026-06-16 Claude review 追修、2026-06-17 固定房間舊 finished 狀態修正、2026-06-17 Claude bug 清單 A/B/C 修正、開始比賽即時回饋修正、主持端清空玩家功能、玩家第一次加入同步保護，以及重設比賽後玩家端計數歸零保護。HTML 目前引用 `styles.css?v=20260617-7` 與 `app.js?v=20260617-7`。
...
目前 GitHub Pages 直接 import `firebase-config.js`，因此這個 Firebase web config 會保留在 repo 中。它不是 server secret；請不要 commit service account JSON、`.env`、私密 token 或其他伺服器憑證。
...
4. 按「開始比賽」。
5. 比賽結束或要重來時按「重設比賽」。
6. 若固定房間有舊玩家或要重新開放報名，按「清空玩家」可移除所有玩家並回到 lobby。
...
- 手機瀏覽器是否載到新版，可在網址後加 `?cache=20260617-7` 或重新掃 QR code。
- 若主持人分頁關閉或當機，請用同一房間重新開主持頁；新版會在舊主持 heartbeat 逾時後接手繼續倒數或推進比賽。
- 玩家手機若停在倒數畫面，請確認已載入 `app.js?v=20260617-7`；新版會在手機端自行更新倒數並在倒數到期後啟用「划！」。
...
- 部署版 HTML 已更新到 `styles.css?v=20260617-7` 與 `app.js?v=20260617-7`。
- 部署版 `styles.css?v=20260617-7` 與 `app.js?v=20260617-7` 可載入。
...
```html
<script type="module" src="./app.js?v=20260617-7"></script>
```

CSS：

```html
<link rel="stylesheet" href="./styles.css?v=20260617-7" />
```
